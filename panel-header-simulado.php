<?php
/*
 * {{REFLECTOR_NAME}} — header compartilhado.
...
if (!is_file($source)) {
    echo '<!-- {{REFLECTOR_NAME}}: fonte do header do Simulado ausente -->';
    return;
}

$html = file_get_contents($source);
if ($html === false) {
    echo '<!-- {{REFLECTOR_NAME}}: falha ao ler header do Simulado -->';
    return;
}

if (!preg_match('~<header\b[^>]*\bclass=(["\'])[^"\']*\bpanel-shell-header\b[^"\']*\1[^>]*>.*?</header>~is', $html, $m)) {
    echo '<!-- {{REFLECTOR_NAME}}: header do Simulado não localizado -->';
    return;
}
