...
function cert_site_config(): array
{
    ...
    if (!is_file($path)) throw new RuntimeException('Configuração do refletor não encontrada.');
    $loaded = require $path;
    if (!is_array($loaded)) throw new RuntimeException('Configuração do refletor inválida.');
...
function cert_timezone(): DateTimeZone
{
    ...
    catch (Throwable $exception) { return new DateTimeZone('UTC'); }
}
...
function cert_campaign(string $id, string $title, string $subtitle, string $theme, DateTimeImmutable $start, DateTimeImmutable $end): array
{
    return [
        'id'=>$id,'title'=>$title,'subtitle'=>$subtitle,'theme'=>$theme,
        'start_ts'=>$start->getTimestamp(),'end_ts'=>$end->getTimestamp(),
        'period_label'=>$start->format('d/m/Y') === $end->format('d/m/Y') ? $start->format('d/m/Y') : $start->format('d/m/Y').' – '.$end->format('d/m/Y'),
    ];
}
...
{
    $site=cert_site_config(); $reflector=cert_reflector(); $tz=cert_timezone();
    $now ??= new DateTimeImmutable('now',$tz); $now=$now->setTimezone($tz);
    $year=(int)$now->format('Y'); $title=$reflector['title']; $specials=[];
    $hamDay=new DateTimeImmutable("{$year}-04-18 00:00:00",$tz);
    $specials[]=cert_campaign("world-amateur-radio-day-{$year}","Dia Mundial do Radioamador {$year}","Participação registrada em {$title}",'radioamador',$hamDay,$hamDay->setTime(23,59,59));
    $certConfig=(array)($site['certificates']??[]); $anniversary=trim((string)($certConfig['anniversary']??''));
    if (preg_match('/^(\d{2})-(\d{2})$/',$anniversary,$match)) {
        $month=(int)$match[1]; $day=(int)$match[2];
        if (checkdate($month,$day,$year)) {
            $official=new DateTimeImmutable(sprintf('%04d-%02d-%02d 00:00:00',$year,$month,$day),$tz);
            $specials[]=cert_campaign('reflector-anniversary-'.strtolower($reflector['name']).'-'.$year,"Semana de Aniversário {$title} {$year}",'Data oficial: '.$official->format('d/m'),'aniversario',$official->modify('-2 days'),$official->modify('+2 days')->setTime(23,59,59));
        }
    }
    if (cert_is_brazil($reflector['country'])) {
        $mother=cert_nth_weekday($year,5,7,2,$tz); $father=cert_nth_weekday($year,8,7,2,$tz);
        $independence=new DateTimeImmutable("{$year}-09-07 00:00:00",$tz); $brRadio=new DateTimeImmutable("{$year}-11-05 00:00:00",$tz);
    ...
    foreach ($specials as $campaign) if ($timestamp >= $campaign['start_ts'] && $timestamp <= $campaign['end_ts']) return $campaign;
    $start=$now->setTime(0,0,0); $end=$now->setTime(23,59,59);
    return cert_campaign('participacao-'.$now->format('Y-m-d'),'Certificado de Participação',"Atividade registrada em {$title}",'participacao',$start,$end);
}
